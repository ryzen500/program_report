...
function generateNoFormulirIKP($kode_rumahsakit) {
    global $conn;
    ...
    // Get the current maximum transaksi_id
    $sql = "SELECT MAX(ikp_laporan_id) AS max_id FROM ikp_laporan";
    $result = $conn->query($sql);
    ...
    if ($result) {
        ...
        if ($row['max_id'] !== null) {
            ...
            error_log("No transactions found in ikp_laporan, starting at 1.");
        }
    } else {
        error_log("Could not fetch max_id value for table ikp_laporan");
    }

    // Generate the noformulir
    $noformulir = "FORMIKP{$next_increment}/{$nama_rumahsakit}/{$bulan}/{$tahun}";
    error_log("Generated noformulir: $noformulir");

    return $noformulir;
}

function generateNoFormulirPelanggan($kode_rumahsakit) {
    global $conn;
    ...
    // Get the current maximum transaksi_id
    $sql = "SELECT MAX(pelanggan_karakteristik_id) AS max_id FROM pelanggan_karakteristik";
    $result = $conn->query($sql);
    ...
    if ($result) {
        ...
        if ($row['max_id'] !== null) {
            ...
            error_log("No transactions found in pelanggan_karakteristik, starting at 1.");
        }
    } else {
        error_log("Could not fetch max_id value for table pelanggan_karakteristik");
    }

    // Generate the noformulir
    $noformulir = "FORMLP{$next_increment}/{$nama_rumahsakit}/{$bulan}/{$tahun}";
    error_log("Generated noformulir: $noformulir");
