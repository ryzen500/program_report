...
const Step9 = ({ prevStep, nextStep, initialData }) => {
    ...
    const fetchSubPelayanan = async () => {
      try {
        ...
        if (initialData) {
          ...
          console.log("Initial data in Step 9:", initialData[0].transaksi_id);
          const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL_BACKEND}/editPoli_transaksi/16/${transaksiId}`, {
            headers: {
          ...
          const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL_BACKEND}/submaster_pelayananDetail/16`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
  ...
  return (
    <div>
      <form onSubmit={handleSubmit(onSubmit, onError)}>
        <Box>
          <Heading as="h1" size="md" mb={4}>Kamar Bersalin</Heading>
