...
const Step5 = ({ prevStep, nextStep, initialData }) => {
    ...
    const fetchSubPelayanan = async () => {
      try {
        ...
        if (initialData) {
          ...
          console.log("Initial data in Step 5:", initialData[0].transaksi_id);
          const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL_BACKEND}/editPoli_transaksi/9/${transaksiId}`, {
            headers: {
          ...
          const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL_BACKEND}/submaster_pelayananDetail/9`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
